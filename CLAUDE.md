...
- **Health alerts:** a source that errors or returns 0 events triggers Telegram (exit code 3), unless it's listed in `KNOWN_BROKEN` in `sync/src/health.ts`. Fix a source → remove it from that list.
